...
socialLogin.factory("fbService", function($q){
	return {
		...
		getUserDetails: function(){
			var deferred = $q.defer();
			FB.api('/me?fields=name,email,picture,friends,friendlists', function(res){
				if(!res || res.error){
				...
				fbService.login().then(function(res){
					if(res.status == "connected"){
						fbService.getUserDetails().then(function(user){
							...
							// console.log(res)
							var userDetails = {token: res.authResponse.accessToken, name: user.name, email: user.email, uid: user.id, provider: "facebook", imageUrl: user.picture.data.url}
							$rootScope.$broadcast('event:social-sign-in-success', userDetails);
						}, function(err){
							console.log(err);
						})
